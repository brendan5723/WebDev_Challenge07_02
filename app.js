...
app.get('/filter', (req, res) => { //filter route
    const filter = req.query.sort; //get the value of the query string
    const burgersSQL=`SELECT id, b_name, price 
    FROM web_dev_burgers ORDER BY ${filter}; `; //SQL query to filter the data

    connection.query(burgersSQL, (err, result)=>{//callback function
        if(err) throw err; //if error, show error
        res.render('menu', { burgerlist : result } );// render the menu.ejs file
    }); //end of callback function
}); //end of route

app.get('/admin/add', (req, res) => { //add route
